...
const App = () => {
  ...
  if (!user && !dbError) return (
    <div className="h-screen flex items-center justify-center bg-transparent text-white font-sans relative overflow-hidden">
      <div className="flex flex-col items-center gap-4 z-10 relative">
        <div className="w-12 h-12 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div>
        <p className="text-slate-300 font-bold tracking-widest uppercase text-sm drop-shadow-md">Menghubungkan ke Cloud...</p>
      </div>
    </div>
  );
  ...
  if (user && !selectedBranch && !dbError) {
    return (
      <div className="min-h-screen bg-transparent text-white font-sans relative overflow-y-auto custom-scrollbar flex flex-col items-center p-6 pb-20">
        ...
        <div className="z-10 relative w-full max-w-4xl text-center mt-4 md:mt-8">
          ...
          <p className="text-slate-300 mb-8 font-medium tracking-widest uppercase text-xs md:text-sm drop-shadow">Sistem Pemusatan Operasi & Aliran Tunai</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-10 w-full animate-in slide-in-from-top-4 duration-500">
             <div className="bg-slate-800/60 backdrop-blur-xl border-t-2 border-slate-700/50 p-4 md:p-5 rounded-2xl md:rounded-[2rem] text-left shadow-lg border-l-4 border-l-blue-500 hover:bg-slate-800/80 transition-all">
                <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-blue-200 mb-1 drop-shadow">Cash / TNG (TamTam)</p>
                <p className="text-xl md:text-2xl font-black text-white tracking-tighter drop-shadow-md">RM {wallets.tamtamCash.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
             </div>
             <div className="bg-slate-800/60 backdrop-blur-xl border-t-2 border-slate-700/50 p-4 md:p-5 rounded-2xl md:rounded-[2rem] text-left shadow-lg border-l-4 border-l-orange-500 hover:bg-slate-800/80 transition-all">
                <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-orange-200 mb-1 drop-shadow">Maybank (TamTam)</p>
                <p className="text-xl md:text-2xl font-black text-white tracking-tighter drop-shadow-md">RM {wallets.tamtamBank.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
             </div>
             <div className="bg-slate-800/60 backdrop-blur-xl border-t-2 border-slate-700/50 p-4 md:p-5 rounded-2xl md:rounded-[2rem] text-left shadow-lg border-l-4 border-l-red-500 hover:bg-slate-800/80 transition-all">
                <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-red-200 mb-1 drop-shadow">Cash / TNG (Bro's Mart)</p>
                <p className="text-xl md:text-2xl font-black text-white tracking-tighter drop-shadow-md">RM {wallets.brosCash.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
             </div>
             <div className="bg-slate-800/60 backdrop-blur-xl border-t-2 border-slate-700/50 p-4 md:p-5 rounded-2xl md:rounded-[2rem] text-left shadow-lg border-l-4 border-l-emerald-500 hover:bg-slate-800/80 transition-all">
                <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-emerald-200 mb-1 drop-shadow">Maybank Raudhah</p>
                <p className="text-xl md:text-2xl font-black text-white tracking-tighter drop-shadow-md">RM {wallets.raudhahBank.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-2xl mx-auto mb-16">
            <button 
              ...
              className="bg-slate-800/60 backdrop-blur-xl border border-slate-700/50 hover:border-orange-500/80 rounded-[2rem] p-8 flex flex-col items-center justify-center transition-all duration-300 group hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(249,115,22,0.5)] active:scale-95 relative overflow-hidden"
            >
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 to-transparent z-0 pointer-events-none"></div>
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-[1.5rem] mb-6 overflow-hidden bg-black p-2 shadow-[0_10px_30px_rgba(0,0,0,0.5)] group-hover:scale-105 transition-transform duration-300 z-10 border border-slate-700">
              ...
              <h2 className="text-xl md:text-2xl font-black tracking-tight text-white mb-2 z-10 drop-shadow-lg">Laporan TamTam</h2>
              <span className="text-[10px] font-black uppercase tracking-widest text-orange-300 bg-orange-500/20 px-4 py-1.5 rounded-xl border border-orange-500/30 z-10">Buka Dashboard</span>
            </button>

            <button 
              onClick={() => { setSelectedBranch('ayam'); setActiveTab('overview'); }} 
              className="bg-slate-800/60 backdrop-blur-xl border border-slate-700/50 hover:border-red-500/80 rounded-[2rem] p-8 flex flex-col items-center justify-center transition-all duration-300 group hover:-translate-y-2 hover:shadow-[0_20px_40px_-15px_rgba(220,38,38,0.5)] active:scale-95 relative overflow-hidden"
            >
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900/80 to-transparent z-0 pointer-events-none"></div>
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-[1.5rem] mb-6 overflow-hidden bg-black shadow-[0_10px_30px_rgba(0,0,0,0.5)] group-hover:scale-105 transition-transform duration-300 flex items-center justify-center relative border border-slate-700 z-10">
              ...
              <h2 className="text-xl md:text-2xl font-black tracking-tight text-white mb-2 z-10 drop-shadow-lg">Laporan Bro's Mart</h2>
              <span className="text-[10px] font-black uppercase tracking-widest text-red-300 bg-red-500/20 px-4 py-1.5 rounded-xl border border-red-500/30 z-10">Buka Dashboard</span>
            </button>
          </div>
          ...
          <div className="w-full max-w-4xl mx-auto bg-slate-800/60 backdrop-blur-2xl border border-slate-600/50 rounded-[2.5rem] p-6 md:p-10 shadow-2xl relative overflow-hidden text-left">
            <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
                <Store size={150} className="text-white" />
            </div>
            
            <h2 className="text-xl md:text-2xl font-black text-white mb-6 tracking-tight text-center relative z-10 drop-shadow">Borang Kemasukan Data</h2>

            {successMsg && (
                <div className="mb-6 bg-emerald-500/20 border border-emerald-500/50 text-emerald-200 px-4 py-3 rounded-xl text-center text-sm font-bold flex justify-center items-center gap-2 animate-in fade-in duration-300 relative z-10 backdrop-blur-sm">
                    <CheckCircle2 size={18} /> {successMsg}
                </div>
            )}
            {formError && (
                <div className="mb-6 bg-rose-500/20 border border-rose-500/50 text-rose-200 px-4 py-3 rounded-xl text-center text-sm font-bold flex justify-center items-center gap-2 animate-in fade-in duration-300 relative z-10 backdrop-blur-sm">
                    <AlertTriangle size={18} /> {formError}
                </div>
            )}

            {/* Form Switcher */}
            <div className="flex bg-slate-900/80 backdrop-blur-md p-1.5 rounded-2xl mb-8 border border-slate-700/80 shadow-inner relative z-10">
                <button onClick={() => setGlobalInputTab('sales')} className={`flex-1 py-3 rounded-xl font-black text-xs transition-all uppercase tracking-widest ${globalInputTab === 'sales' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-200'}`}>Jualan</button>
                <button onClick={() => setGlobalInputTab('expenses')} className={`flex-1 py-3 rounded-xl font-black text-xs transition-all uppercase tracking-widest ${globalInputTab === 'expenses' ? 'bg-rose-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-200'}`}>Belanja</button>
                <button onClick={() => setGlobalInputTab('transfers')} className={`flex-1 py-3 rounded-xl font-black text-xs transition-all uppercase tracking-widest ${globalInputTab === 'transfers' ? 'bg-emerald-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-200'}`}>Pindahan</button>
            </div>
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-3 mb-8 relative z-10">
                   <button
                     ...
                     className={`px-6 py-3 sm:py-2 rounded-xl sm:rounded-full font-black text-xs transition-all border ${
                         ...
                         ? 'bg-orange-500 border-orange-400 text-white shadow-[0_0_20px_rgba(249,115,22,0.6)]'
                         : 'bg-slate-900/80 backdrop-blur-sm border-slate-700 text-slate-400 hover:border-slate-500 hover:text-white'
                     }`}
                 >
                     REKOD KE: TAM-TAM
                 </button>
            ...
            {globalInputTab === 'sales' && (
                <div className="space-y-5 animate-in slide-in-from-bottom-4 duration-300 relative z-10">
                    <div>
                      <label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2 drop-shadow">Tarikh Jualan</label>
                      <input type="date" value={formSales.date} onChange={(e) => setFormSales({...formSales, date: e.target.value})} className="w-full bg-slate-900/80 backdrop-blur-md border border-slate-600 rounded-xl px-4 py-3 font-black text-white outline-none focus:border-blue-500 shadow-inner text-sm transition-colors" />
                    </div>
                    ...
                    <div className="mt-4 pt-2">
                       <button onClick={handleSaveSales} className="w-full bg-blue-600/90 backdrop-blur-md text-white py-4 rounded-xl font-black shadow-[0_10px_20px_rgba(37,99,235,0.4)] hover:bg-blue-500 transition-all flex items-center justify-center gap-2 active:scale-95 text-sm uppercase tracking-widest border border-blue-400/50"><Save size={18} /> Simpan Data Jualan</button>
                    </div>
                </div>
            )}
            ...
            {globalInputTab === 'expenses' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 animate-in slide-in-from-bottom-4 duration-300 relative z-10">
                    <div className="md:col-span-2">
                        <button onClick={() => setIsScannerModalOpen(true)} className="w-full flex items-center justify-center gap-2 bg-indigo-500/20 backdrop-blur-sm text-indigo-200 hover:bg-indigo-500/30 hover:text-white px-4 py-3 rounded-xl font-black text-xs transition-all border border-indigo-400/40 shadow-sm active:scale-95 uppercase tracking-widest">
                            <Camera size={16} /> Imbas Resit / Invois (AI)
                        </button>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2 drop-shadow">Tarikh Belanja</label>
                      <input type="date" value={formExpense.date} onChange={(e) => setFormExpense({...formExpense, date: e.target.value})} className="w-full bg-slate-900/80 backdrop-blur-md border border-slate-600 rounded-xl px-4 py-3 font-black text-white outline-none focus:border-rose-500 shadow-inner text-sm transition-colors" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2 drop-shadow">Kategori</label>
                      <select value={formExpense.category} onChange={(e) => setFormExpense({...formExpense, category: e.target.value})} className="w-full bg-slate-900/80 backdrop-blur-md border border-slate-600 rounded-xl py-3 px-4 text-sm font-black text-white outline-none focus:border-rose-500 shadow-inner appearance-none transition-colors">
                         <option value="Kos Mentah (COGS)">Kos Mentah / Stok (COGS)</option>
                         <option value="Gaji & Operasi (OPEX)">Gaji, Bil & Operasi (OPEX)</option>
                      </select>
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2 drop-shadow">Cara Bayaran (Sumber Dompet)</label>
                      <select value={formExpense.payment_method} onChange={(e) => setFormExpense({...formExpense, payment_method: e.target.value})} className="w-full bg-slate-900/80 backdrop-blur-md border border-slate-600 rounded-xl py-3 px-4 text-sm font-black text-white outline-none focus:border-rose-500 shadow-inner appearance-none transition-colors">
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2 drop-shadow">Rujukan / Catatan</label>
                      <textarea value={formExpense.description} onChange={(e) => setFormExpense({...formExpense, description: e.target.value})} className="w-full bg-slate-900/80 backdrop-blur-md border border-slate-600 rounded-xl p-4 text-sm font-bold text-white shadow-inner h-24 custom-scrollbar focus:border-rose-500 outline-none transition-colors" placeholder="Cth: NSK Beli Barang Basah"></textarea>
                    </div>
                    <div className="md:col-span-2 mt-4 pt-2">
                       <button onClick={handleSaveExpense} className="w-full bg-rose-600/90 backdrop-blur-md text-white py-4 rounded-xl font-black shadow-[0_10px_20px_rgba(225,29,72,0.4)] hover:bg-rose-500 transition-all flex items-center justify-center gap-2 active:scale-95 text-sm uppercase tracking-widest border border-rose-400/50"><Save size={18} /> Simpan Perbelanjaan</button>
                    </div>
                </div>
            )}
            ...
            {globalInputTab === 'transfers' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 animate-in slide-in-from-bottom-4 duration-300 relative z-10">
                    <div className="md:col-span-2 bg-emerald-500/20 backdrop-blur-sm p-4 border border-emerald-400/40 rounded-xl flex items-start gap-3 text-emerald-100 text-xs shadow-inner">
                    ...
                    <div>
                      <label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2 drop-shadow">Tarikh Pindahan</label>
                      <input type="date" value={formTransfer.date} onChange={(e) => setFormTransfer({...formTransfer, date: e.target.value})} className="w-full bg-slate-900/80 backdrop-blur-md border border-slate-600 rounded-xl px-4 py-3 font-black text-white outline-none focus:border-emerald-500 shadow-inner text-sm transition-colors" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2 drop-shadow">Jenis Pindahan</label>
                      <select value={formTransfer.type} onChange={(e) => setFormTransfer({...formTransfer, type: e.target.value, payer: ''})} className="w-full bg-slate-900/80 backdrop-blur-md border border-slate-600 rounded-xl py-3 px-4 text-sm font-black text-white outline-none focus:border-emerald-500 shadow-inner appearance-none transition-colors">
                    ...
                    {['modal_luar_cash', 'modal_luar_bank', 'bayar_hutang_luar', 'modal_luar_cash_bros', 'modal_luar_bank_bros', 'bayar_hutang_luar_bros'].includes(formTransfer.type) && (
                      <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                         <div>
                            ...
                            <select 
                                ...
                                className="w-full bg-slate-900/80 backdrop-blur-md border border-slate-600 rounded-xl py-3 px-4 text-sm font-black text-white outline-none focus:border-emerald-500 shadow-inner appearance-none transition-colors"
                            >
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2 drop-shadow">Rujukan / Catatan</label>
                      <textarea value={formTransfer.description} onChange={(e) => setFormTransfer({...formTransfer, description: e.target.value})} className="w-full bg-slate-900/80 backdrop-blur-md border border-slate-600 rounded-xl p-4 text-sm font-bold text-white shadow-inner h-20 custom-scrollbar focus:border-emerald-500 outline-none transition-colors" placeholder="Cth: Bank-in duit jualan hujung minggu"></textarea>
                    </div>
                    <div className="md:col-span-2 mt-4 pt-2">
                       <button onClick={handleSaveTransfer} className="w-full bg-emerald-600/90 backdrop-blur-md text-white py-4 rounded-xl font-black shadow-[0_10px_20px_rgba(5,150,105,0.4)] hover:bg-emerald-500 transition-all flex items-center justify-center gap-2 active:scale-95 text-sm uppercase tracking-widest border border-emerald-400/50"><CheckCircle2 size={18} /> Sahkan Pindahan</button>
                    </div>
                </div>
            )}
          </div>
        ...
        >
          <source src="https://strvid.nyc3.digitaloceanspaces.com/motionitems/source/1782054601597-motion_79.mp4" type="video/mp4" />
        </video>
        {/* Dark overlay specifically for the global view to ensure text readability */}
        <div className="fixed inset-0 bg-slate-900/70 z-[-1] pointer-events-none"></div>
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen font-sans text-slate-900 overflow-hidden relative">

      {/* Video Background Layer for Branch View */}
      <video 
        autoPlay 
        loop 
        muted 
      ...
      {/* Light Overlay for Branch Dashboard view to maintain readability */}
      <div className="absolute inset-0 bg-slate-50/90 backdrop-blur-[5px] pointer-events-none z-0"></div>

      <input type="file" ref={fileInputRef} className="hidden relative z-10" accept=".csv" onChange={handleFileChange} />
      ...
      {/* Mobile Header */}
      <div className="md:hidden flex justify-between items-center bg-slate-900/95 backdrop-blur-md text-white p-4 z-30 shrink-0 shadow-md relative border-b border-slate-800/50">
        <div className="flex items-center gap-3">
      ...
      {/* Sidebar Responsive */}
      <div className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900/95 backdrop-blur-2xl text-white flex flex-col p-4 overflow-y-auto transition-transform duration-300 ease-in-out md:static md:translate-x-0 md:shrink-0 border-r border-slate-800/50 shadow-2xl relative ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex justify-between items-start mb-8 px-2">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black leading-none tracking-tight text-white font-sans uppercase tracking-tighter drop-shadow-md">
        ...
        <button onClick={handleLogoutBranch} className="mb-8 w-full flex items-center justify-center gap-2 py-3 bg-slate-800/80 backdrop-blur-md text-slate-300 rounded-xl hover:bg-slate-700 hover:text-white transition-all text-xs font-black uppercase tracking-widest shadow-inner border border-slate-700/50">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar relative z-10">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 gap-4">
          <div>
            <h2 className="text-xl md:text-2xl font-black uppercase tracking-tight text-slate-800 drop-shadow-sm">
               {activeTab === 'sales' ? 'REKOD JUALAN' : activeTab === 'expenses' ? 'REKOD BELANJA' : activeTab.replace('-', ' ')}
            </h2>
            <p className="text-slate-600/80 text-xs md:text-[13px] font-bold italic font-sans">
              "Laporan {selectedBranch === 'tamtam' ? 'Burger' : "Bro's Mart"} &bull; TamTam ERP"
            </p>
          </div>
          <div className="flex flex-wrap gap-2 md:gap-3 w-full sm:w-auto">
            <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-white/90 backdrop-blur-sm border border-slate-200/50 px-3 py-2 md:px-5 md:py-2.5 rounded-xl text-[10px] md:text-sm font-black shadow-sm hover:bg-white transition-all">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-4 md:space-y-6 animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6">
               <div className="xl:col-span-2 bg-white/80 backdrop-blur-xl p-4 md:p-8 rounded-2xl md:rounded-[2rem] shadow-lg border border-white/50 h-64 md:h-80 flex flex-col">
                 ...
                 <div className="flex-1 flex items-end justify-between gap-2 md:gap-4 pt-4 border-b border-slate-200/50 pb-2 h-full">
                   {(() => {
                     const last7Days = (salesData || []).slice(-7);
                     if (last7Days.length === 0) return <div className="w-full h-full flex items-center justify-center text-slate-500 text-xs font-bold italic">Tiada rekod data jualan</div>;
                     ...
                     return last7Days.map((day, idx) => {
                       ...
                       return (
                          <div key={day.id || idx} className="flex flex-col items-center justify-end h-full w-full gap-2 group relative">
                             ...
                             <div className="w-full max-w-[40px] bg-slate-100 rounded-t-md relative flex flex-col justify-end overflow-hidden border-b-2 border-transparent group-hover:border-blue-500 transition-all shadow-inner" style={{ height: `${Math.max(hTotal, 1)}%` }}>
                             ...
                             <span className="text-[8px] md:text-[10px] text-slate-500 font-bold truncate">{day.date ? day.date.substring(5) : ''}</span>
                          </div>
                       );
                     });
                   })()}
                 </div>
                 <div className="flex flex-wrap justify-center gap-3 md:gap-5 mt-4 text-[9px] md:text-[10px] font-bold text-slate-600 uppercase tracking-widest">
               ...
               <div className="bg-slate-900/90 backdrop-blur-xl p-6 md:p-8 rounded-2xl md:rounded-[2rem] text-white shadow-2xl relative overflow-hidden flex flex-col border border-slate-700/50">
                 <div className="absolute bottom-0 right-0 p-8 opacity-5"><DollarSign size={100} className="md:w-[120px] md:h-[120px]"/></div>
                 <h3 className="text-lg font-black mb-2 md:mb-6 tracking-tight uppercase text-blue-400 drop-shadow">Prestasi Keseluruhan</h3>
                 <div className="space-y-4 flex-1 mt-4 relative z-10">
                   ...
                   <div className="pt-2">
                      ...
                      <p className="text-3xl font-black text-emerald-400 tracking-tighter drop-shadow-md">RM {stats.netProfit.toLocaleString(undefined, {minimumFractionDigits: 2})}</p>
                   </div>
                 </div>
               </div>
            </div>

            <div className="bg-white/90 backdrop-blur-xl rounded-2xl md:rounded-[2rem] shadow-lg border border-white/50 overflow-hidden">
              <div className="p-5 md:p-8 border-b border-slate-200/50 bg-slate-50/50">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left min-w-[700px]">
                  <thead className="bg-slate-50/50 text-slate-500 text-[9px] md:text-[10px] uppercase tracking-[0.2em] font-black border-b border-slate-200/50">
                    <tr>
                      ...
                      <th className="px-4 md:px-8 py-4 md:py-5 text-right text-blue-700 bg-blue-50/30">Total Jualan</th>
                      <th className="px-4 md:px-8 py-4 md:py-5 text-right text-rose-600 bg-rose-50/30">Perbelanjaan</th>
                      <th className="px-4 md:px-8 py-4 md:py-5 text-right text-emerald-700 bg-emerald-50/30">Untung Bersih</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200/50 font-bold text-xs md:text-sm">
                    {(monthlyStats || []).map(m => {
                      ...
                      return (
                        <tr key={m.month} className="hover:bg-slate-50 transition-all">
                          ...
                          <td className="px-4 md:px-8 py-4 md:py-6 text-right text-slate-600 whitespace-nowrap">RM {m.walkin.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                          <td className="px-4 md:px-8 py-4 md:py-6 text-right text-slate-600 whitespace-nowrap">RM {m.apps.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                          <td className="px-4 md:px-8 py-4 md:py-6 text-right font-black text-blue-700 bg-blue-50/30 whitespace-nowrap">RM {m.earning.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                          <td className="px-4 md:px-8 py-4 md:py-6 text-right font-black text-rose-600 bg-rose-50/30 whitespace-nowrap">RM {m.expense.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                    ...
                    {(!monthlyStats || monthlyStats.length === 0) && <tr><td colSpan="6" className="text-center py-8 text-slate-500 italic text-[10px] uppercase tracking-widest font-black">Tiada rekod</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
        ...
        {activeTab === 'sales' && (
          <div className="space-y-6 md:space-y-8 animate-in slide-in-from-bottom-4 duration-300">
            <div className="bg-white/90 backdrop-blur-xl rounded-2xl md:rounded-[2.5rem] shadow-lg border border-white/50 overflow-hidden flex flex-col">
              <div className="p-5 md:p-8 border-b border-slate-200/50 bg-slate-50/50 flex justify-between items-center">
              ...
              <div className="overflow-x-auto flex-1">
                <table className="w-full text-left relative min-w-[800px]">
                  <thead className="bg-white/50 text-slate-500 text-[9px] md:text-[10px] uppercase tracking-[0.2em] font-black border-b border-slate-200/50 sticky top-0 z-10 backdrop-blur-md">
                    <tr>
                      ...
                      <th className="px-4 md:px-6 py-4 md:py-5 text-right text-blue-700">Total Jualan</th>
                      <th className="px-4 md:px-6 py-4 md:py-5 text-center">Aksi</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200/50 font-bold text-[11px] md:text-xs">
                      ...
                      return (
                        <tr key={day.id} className="hover:bg-slate-50 transition-all group">
                          ...
                          <td className="px-4 md:px-6 py-4 md:py-5">
                            {day.source ? <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded border border-blue-200 text-[9px] uppercase tracking-widest shadow-sm">{day.source.substring(0,15)}...</span> : <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded border border-slate-300 text-[9px] uppercase tracking-widest shadow-sm">Manual</span>}
                          </td>
                          <td className="px-4 md:px-6 py-4 md:py-5 text-right text-slate-600 whitespace-nowrap tracking-tighter">RM {walkin.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                          <td className="px-4 md:px-6 py-4 md:py-5 text-right text-slate-600 whitespace-nowrap tracking-tighter">RM {apps.toLocaleString(undefined, {minimumFractionDigits: 2})}</td>
                          ...
                          <td className="px-4 md:px-6 py-4 md:py-5 text-center">
                            <button onClick={() => setSaleToDelete(day.id)} className="text-slate-400 hover:text-rose-600 transition-colors p-2 active:scale-90 bg-white border border-slate-200 rounded-md shadow-sm">
                              <Trash2 size={16}/>
                            </button>
                          </td>
                        </tr>
                    ...
                    {(!salesData || salesData.length === 0) && <tr><td colSpan={6} className="text-center py-10 text-slate-500 italic uppercase tracking-widest text-[10px]">Tiada rekod jualan</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
        ...
        {activeTab === 'expenses' && (
          <div className="space-y-6 md:space-y-8 animate-in slide-in-from-bottom-4 duration-300">
            <div className="bg-white/90 backdrop-blur-xl rounded-[2.5rem] border border-white/50 overflow-hidden shadow-lg flex flex-col h-fit">
              <div className="p-6 md:p-8 bg-slate-50/50 border-b border-slate-200/50 flex justify-between items-center"><h3 className="font-black text-xl uppercase tracking-tight text-slate-800">Semua Log Perbelanjaan</h3></div>
              <div className="overflow-x-auto flex-1">
                <table className="w-full text-left min-w-[700px]"><thead className="bg-white/50 text-slate-500 text-[9px] uppercase tracking-[0.2em] font-black sticky top-0 border-b border-slate-200/50 backdrop-blur-md"><tr><th className="px-6 py-4 md:py-5">Tarikh</th><th className="px-6 py-4 md:py-5">Kategori</th><th className="px-6 py-4 md:py-5">Catatan</th><th className="px-6 py-4 md:py-5 text-right">Jumlah RM</th><th className="px-6 py-4 md:py-5 text-center">Aksi</th></tr></thead>
                  <tbody className="divide-y divide-slate-200/50 font-bold text-[11px] md:text-xs">{([...expensesData] || []).reverse().map(exp => {
                      ...
                      <tr key={exp.id} className="hover:bg-slate-50 transition-all">
                        ...
                        <td className="px-6 py-4 md:py-5">
                           <div className="flex flex-col gap-1">
                             <span className={`w-fit px-2 py-1 rounded border text-[9px] uppercase tracking-widest shadow-sm ${isCogs ? 'bg-orange-100 text-orange-700 border-orange-200' : 'bg-purple-100 text-purple-700 border-purple-200'}`}>
                                {isCogs ? 'Kos Mentah' : 'Operasi'}
                             </span>
                             <span className={`w-fit px-2 py-0.5 rounded text-[8px] uppercase tracking-widest border shadow-sm ${exp.payment_method === 'bank' ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-slate-200 text-slate-700 border-slate-300'}`}>
                                Byr: {exp.payment_method === 'bank' ? 'Bank' : 'Tunai Laci'}
                             </span>
                           </div>
                        </td>
                        <td className="px-6 py-4 md:py-5 text-slate-700 max-w-[300px] break-words leading-relaxed">{exp.description || '-'}</td>
                        <td className="px-6 py-4 md:py-5 text-right text-rose-600 font-black whitespace-nowrap text-sm">RM {Number(exp.amount).toFixed(2)}</td>
                        <td className="px-6 py-4 md:py-5 text-center"><button onClick={() => setExpenseToDelete(exp.id)} className="text-slate-400 hover:text-rose-600 transition-colors p-2 active:scale-90 bg-white border border-slate-200 rounded-md shadow-sm"><Trash2 size={16} /></button></td>
                      </tr>
                  )})}
                  {(!expensesData || expensesData.length === 0) && <tr><td colSpan="5" className="text-center py-10 text-slate-500 italic uppercase tracking-widest text-[10px]">Tiada rekod perbelanjaan</td></tr>}
                  </tbody>
                </table>
              </div>
            </div>
        ...
        {activeTab === 'inventory' && (
          <div className="space-y-6 md:space-y-8 animate-in slide-in-from-bottom-4 duration-300">
            <div className="bg-white/90 backdrop-blur-xl p-5 md:p-8 rounded-2xl md:rounded-[2.5rem] shadow-lg border border-white/50">
            ...
            {(!inventory || inventory.length === 0) ? (
               <div className="bg-white/80 backdrop-blur-md p-10 rounded-[2.5rem] border-2 border-dashed border-slate-300 flex flex-col items-center justify-center text-center shadow-sm"><Package size={48} className="text-slate-400 mb-4" /><h4 className="text-lg font-black text-slate-700 uppercase tracking-widest">Tiada Item Stok</h4></div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
                {(inventory || []).map(item => (
                  <div key={item.id} className={`p-5 md:p-8 rounded-2xl md:rounded-[2.5rem] border-2 transition-all relative group shadow-md backdrop-blur-md ${item.stock < 10 ? 'bg-rose-50/90 border-rose-300' : 'bg-white/90 border-white/50'}`}>
                    <button onClick={() => setInventoryToDelete(item.id)} className="absolute top-4 right-4 md:top-6 md:right-6 p-2 bg-white border border-slate-200 rounded-lg text-slate-400 hover:text-rose-600 shadow-sm opacity-0 group-hover:opacity-100 transition-all active:scale-90 z-10"><Trash2 size={14}/></button>
                    <div className="flex justify-between items-start mb-4 md:mb-6 pr-8">
                      <span className={`text-[10px] font-black uppercase tracking-widest ${item.stock < 10 ? 'text-rose-700' : 'text-slate-500'}`}>{item.item}</span>
                      {item.stock < 10 && <AlertTriangle size={18} className="text-rose-600 animate-pulse" />}
                    </div>
                    <div className="text-3xl md:text-4xl font-black text-slate-900 mb-6 md:mb-8 tracking-tighter drop-shadow-sm">{item.stock} <span className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-tighter">{item.unit}</span></div>
                    <div className="space-y-3 md:space-y-4">
                      <input type="number" placeholder="Jumlah" value={manualStockInputs[item.id] || ''} onChange={(e) => setManualStockInputs({...manualStockInputs, [item.id]: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2 font-black text-xs md:text-sm text-slate-800 outline-none focus:border-blue-500 shadow-inner transition-all" />
            ...
            <div className="bg-white/90 backdrop-blur-xl p-5 md:p-10 rounded-2xl md:rounded-[2.5rem] shadow-lg border border-white/50">
               ...
               <div className="space-y-3 md:space-y-4">
                 {(stockLogs || []).map(log => (
                   <div key={log.id} className="flex items-center justify-between p-3 md:p-5 bg-white border border-slate-100 rounded-xl md:rounded-2xl shadow-sm">
                     <div className="flex items-center gap-3 md:gap-5">
                        <div className={`w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center font-black text-sm md:text-base shadow-inner ${log.type === 'restock' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>{log.type === 'restock' ? '+' : '-'}</div>
                        <div>
                          <p className="font-black text-slate-800 tracking-tight text-xs md:text-sm">{log.type === 'restock' ? 'Restock' : 'Wastage'}: {log.itemName}</p>
                          <p className="text-[8px] md:text-[10px] text-slate-500 font-bold uppercase tracking-widest">{log.timestamp?.seconds ? new Date(log.timestamp.seconds * 1000).toLocaleString('ms-MY', { day: 'numeric', month: 'short', hour: '2-digit', minute:'2-digit' }) : 'Memproses...'}</p>
                        </div>
                     </div>
                 ...
                 {(!stockLogs || stockLogs.length === 0) && <p className="text-center text-[10px] text-slate-500 uppercase font-black italic">Tiada log stok</p>}
               </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'cashflow' && (
          <div className="space-y-6 md:space-y-8 animate-in slide-in-from-bottom-4 duration-300">
             <div className="grid grid-cols-2 gap-4 md:gap-6 w-full">
                {selectedBranch === 'tamtam' ? (
                  <>
                    <div className="bg-white/90 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] text-left shadow-lg border border-white/50">
                       <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-2"><Wallet size={16} className="text-blue-600"/> Cash / TNG (TamTam)</p>
                       <p className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter drop-shadow-sm">RM {wallets.tamtamCash.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
                    </div>
                    <div className="bg-white/90 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] text-left shadow-lg border border-white/50 relative overflow-hidden">
                       <div className="absolute top-0 right-0 p-4 opacity-5"><Lock size={64} className="text-orange-500" /></div>
                       <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-2"><Store size={16} className="text-orange-600"/> Maybank (TamTam)</p>
                       <p className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter relative z-10 drop-shadow-sm">RM {wallets.tamtamBank.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
                    </div>
                  </>
                ) : (
                  <>
                    <div className="bg-white/90 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] text-left shadow-lg border border-white/50">
                       <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-2"><Wallet size={16} className="text-red-600"/> Cash / TNG (Bro's Mart)</p>
                       <p className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter drop-shadow-sm">RM {wallets.brosCash.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
                    </div>
                    <div className="bg-white/90 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] text-left shadow-lg border border-white/50 relative overflow-hidden">
                       <div className="absolute top-0 right-0 p-4 opacity-5"><Lock size={64} className="text-emerald-500" /></div>
                       <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 flex items-center gap-2"><Store size={16} className="text-emerald-600"/> Maybank Raudhah</p>
                       <p className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter relative z-10 drop-shadow-sm">RM {wallets.raudhahBank.toLocaleString(undefined, {minimumFractionDigits:2})}</p>
                    </div>
                  </>
                )}
             </div>
                 ...
                 <div className="mt-8">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                       {hutangLuarBreakdown.map((f, idx) => (
                          <div key={idx} className="bg-white/90 backdrop-blur-xl p-4 md:p-6 rounded-2xl shadow-lg border border-white/50 border-l-[6px] border-l-slate-700">
             ...
             <div className="bg-white/90 backdrop-blur-xl rounded-[2.5rem] border border-white/50 overflow-hidden shadow-lg flex flex-col h-fit mt-8">
               <div className="p-6 md:p-8 bg-slate-50/50 border-b border-slate-200/50 flex justify-between items-center"><h3 className="font-black text-xl uppercase tracking-tight text-slate-800">Log Pindahan Wang (Ledger)</h3></div>
               <div className="overflow-x-auto flex-1">
                     ...
                     {([...transfersData] || []).reverse().filter(t => 
                       ...
                       return (
                         <tr key={trans.id} className="hover:bg-slate-50 transition-all">
                           ...
                           <td className="px-6 py-4 md:py-5">
                              <span className="px-2 py-1 rounded border text-[9px] uppercase tracking-widest bg-white text-slate-700 border-slate-300 shadow-sm">
                                 {trans.type.replace(/_/g, ' ')}
                              </span>
                           </td>
                           <td className="px-6 py-4 md:py-5 text-slate-700 max-w-[300px] break-words leading-relaxed">{trans.description || '-'}</td>
                           <td className={`px-6 py-4 md:py-5 text-right font-black whitespace-nowrap text-sm ${trans.type.includes('draw') ? 'text-rose-600' : 'text-blue-600'}`}>RM {Number(trans.amount).toFixed(2)}</td>
                           <td className="px-6 py-4 md:py-5 text-center"><button onClick={() => setTransferToDelete(trans.id)} className="text-slate-400 hover:text-rose-600 transition-colors p-2 active:scale-90 bg-white border border-slate-200 rounded-md shadow-sm"><Trash2 size={16} /></button></td>
                         </tr>
                     )})}
                     {(!transfersData || transfersData.length === 0) && <tr><td colSpan="5" className="text-center py-10 text-slate-500 italic uppercase tracking-widest text-[10px]">Tiada rekod pindahan wang / ledger</td></tr>}
                   </tbody>
                 </table>
               </div>
             </div>
        ...
        {activeTab === 'import' && (
           <div className="max-w-4xl mx-auto py-4 md:py-8">
            {!pendingSyncData ? (
              <div className="bg-white/90 backdrop-blur-xl p-6 md:p-12 rounded-3xl md:rounded-[2.5rem] border-4 border-dashed border-slate-300 flex flex-col items-center shadow-lg relative overflow-hidden text-center">
                {uploadSuccess && (
                  <div className="absolute inset-0 bg-white/95 flex flex-col items-center justify-center animate-in fade-in duration-300 z-10 p-6">
                    ...
                    <p className="text-sm text-slate-600 mb-8 font-bold italic">Kategori lain telah diasingkan secara automatik.</p>
                ...
                <div className="w-16 h-16 md:w-20 md:h-20 bg-blue-100 text-blue-600 rounded-[1.5rem] flex items-center justify-center mb-6 shadow-inner rotate-3">
                ...
                <p className="text-xs md:text-sm text-slate-600 mb-6 font-medium max-w-sm italic">Sila muat naik CSV 'Item Sales' Loyverse. Sistem menapis Category "{expectedCategory}" untuk bahagian ini secara automatik.</p>
                
                <div className="bg-white p-4 md:p-6 rounded-2xl border border-slate-200 mb-6 md:mb-8 w-full max-w-md shadow-sm">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 gap-2">
                    <label className="block text-[10px] font-black text-slate-600 uppercase tracking-widest text-left">Tarikh Rekod</label>
                    <button 
                      onClick={() => setImportDate(new Date().toISOString().split('T')[0])}
                      className="text-[9px] font-black text-blue-700 bg-blue-100 px-3 py-1.5 rounded-lg hover:bg-blue-200 transition-colors shadow-sm active:scale-95 self-start sm:self-auto"
                    >Tarikh Terkini</button>
                  </div>
                  <input type="date" value={importDate} onChange={(e) => setImportDate(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 px-4 font-black text-slate-800 outline-none focus:border-blue-500 shadow-inner transition-all mb-3 cursor-pointer text-sm" />
                </div>
              ...
              <div className="space-y-4 md:space-y-6 animate-in slide-in-from-bottom-4 duration-300">
                ...
                <div className="bg-white/90 backdrop-blur-xl rounded-2xl md:rounded-[2rem] shadow-lg border border-white/50 overflow-hidden">
                   <div className="p-4 md:p-6 border-b border-slate-200/50 bg-slate-50/50 flex justify-between items-center">
                     ...
                     <button onClick={() => setPendingSyncData(null)} className="text-slate-400 hover:text-rose-600 hover:bg-white p-1.5 md:p-2 rounded-xl transition-all active:scale-90 border border-slate-200 shadow-sm"><X size={18} strokeWidth={3} /></button>
                   </div>
                   <div className="overflow-x-auto max-h-[300px] md:max-h-[400px]">
                      <table className="w-full text-left min-w-[400px]">
                        <thead className="bg-white/50 text-[9px] uppercase font-black tracking-widest text-slate-500 sticky top-0 backdrop-blur-md border-b border-slate-200/50"><tr><th className="px-4 py-4">Action</th><th className="px-4 py-4">Item</th><th className="px-4 py-4 text-right">Net Sales</th></tr></thead>
                        <tbody className="divide-y divide-slate-200/50 font-bold text-xs">
                          {(pendingSyncData.rawItems || []).map((item, idx) => {
                            const isIncluded = item.category && item.category.toUpperCase().includes(expectedCategory);
                            return (
                            <tr key={idx} className={!isIncluded ? 'opacity-40 grayscale italic bg-slate-100/50' : 'hover:bg-slate-50/50 transition-colors'}>
                              <td className="px-4 py-4">{isIncluded ? <div className="text-emerald-700 text-[9px] uppercase tracking-widest bg-emerald-100 px-2 py-1 rounded w-fit shadow-sm">Included</div> : <div className="text-slate-500 text-[9px] uppercase tracking-widest">Skip</div>}</td>
                              <td className="px-4 py-4 text-slate-800">{item.item}</td>
                              <td className="px-4 py-4 text-right text-slate-700">RM {Number(item.net_sales || 0).toFixed(2)}</td>
                            </tr>
                          )})}
                        </tbody>
                      </table>
                   </div>
                   <div className="p-4 md:p-6 bg-slate-100/50 border-t border-slate-200/50 flex flex-col sm:flex-row justify-end gap-3">
                      <button onClick={() => setPendingSyncData(null)} className="px-6 py-3 rounded-xl border border-slate-300 bg-white font-black text-slate-600 shadow-sm hover:bg-slate-50">Batal</button>
                      <button onClick={handleConfirmSync} className="px-6 py-3 rounded-xl bg-slate-900 text-white font-black shadow-lg hover:bg-black"><Check size={18} className="inline mr-2"/> Sah & Sync Jualan</button>
                   </div>
                </div>
              </div>
            )}
...
const SidebarItem = ({ icon, label, active, onClick }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/30' : 'text-slate-400 hover:text-white hover:bg-slate-800/80 border border-transparent hover:border-slate-700/50'}`}>
...
const StatCard = ({ title, value, color, progress, icon }) => {
  const styles = { blue: 'border-l-blue-500 bg-white/90 text-blue-700', emerald: 'border-l-emerald-500 bg-white/90 text-emerald-700', orange: 'border-l-orange-500 bg-white/90 text-orange-700', purple: 'border-l-purple-500 bg-white/90 text-purple-700', slate: 'border-l-slate-700 bg-white/90 text-slate-800' };
  return (
    <div className={`backdrop-blur-xl p-6 md:p-8 rounded-[2rem] border border-white/50 border-l-[10px] ${styles[color]} shadow-lg transition-transform hover:-translate-y-1`}>
      <div className="flex justify-between items-start mb-4">
        <h4 className="text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] leading-relaxed drop-shadow-sm">{title}</h4>
        {icon && <div className="text-slate-400">{icon}</div>}
      </div>
      <div className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter drop-shadow-sm">{value}</div>
      {progress !== undefined && <div className="mt-4 md:mt-6 w-full bg-slate-200 h-2 md:h-2.5 rounded-full overflow-hidden border border-slate-300 shadow-inner"><div className="bg-orange-500 h-full shadow-[0_0_15px_rgba(249,115,22,0.6)] transition-all duration-1000" style={{ width: `${Math.min(progress, 100)}%` }}></div></div>}
    </div>
  );
};
...
const InputField = ({ label, prefix, value, onChange, placeholder = "0", type }) => {
  ...
  return (
    <div>
      <label className="block text-[10px] font-black text-slate-600 mb-2 uppercase tracking-widest">{label}</label>
      <div className="relative">
        {hasPrefix && <span className="absolute left-5 inset-y-0 flex items-center text-slate-400 font-black">{prefix}</span>}
        <input type={inputType} value={value} onChange={(e) => onChange(e.target.value)} className={`w-full bg-white border border-slate-200 rounded-2xl py-4 pl-12 pr-5 font-black text-slate-800 outline-none focus:border-blue-500 shadow-sm transition-all`} placeholder={placeholder} />
      </div>
    </div>
  );
};
